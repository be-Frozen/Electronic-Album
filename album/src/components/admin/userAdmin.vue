    ...
    <div class="content-container" direction="vertical">
        ...
        <div class="show" style="height: 98%;overflow: auto">
            ...
            <el-table
                ...
                :data="this.$store.state.user_info"
                tooltip-effect="dark"
                height="750"
                empty-text="没有数据"
                style="width: 100%">
                <el-table-column
                    ...
                    <template #default="scope">
                        <div style="text-align:left"><el-image :src="'/myApi'+'album/头像/'+scope.row['head']" style="width: 60px; height: 60px"/></div>
                    </template>
...
export default {
    ...
    // 组件挂载时获取数据
    mounted() {
        let phpaddr = "album/php/getUser.php";
        var xmlhttp;
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                var x = JSON.parse(xmlhttp.responseText);
                Storage.commit('getUserInfo',x);
                console.log(x);
            }
        }
        xmlhttp.open("GET", '/myApi' + phpaddr, true);
        xmlhttp.send();
    ...
    methods: {
        ...
        deleteUser(index){
            var name=Storage.state.user_info[index]['name'];
            this.$confirm(
                '确认删除该用户吗？（注意，该操作不可逆。）',
                '警告',
                {
                ...
                .then(() => {
                    let phpaddr = "album/php/deleteUser.php";
                    var albumxmlhttp;
                    ...
                    albumxmlhttp.open("GET", '/myApi' + phpaddr + '?name='+name, true);
                    albumxmlhttp.send();

                    var arr=[].concat(Storage.state.user_info);
                    arr.splice(index,1);
                    Storage.commit('getUserInfo',arr);
                })
                .catch(() => {
                    // catch error
                });
